...
async function calcularSistema() {
  ...
  try {
    ...
    const longitud = Number.parseFloat(document.getElementById("longitud").value)
    const eficiencia = Number.parseFloat(document.getElementById("eficiencia").value)
    const potenciaPanel = Number.parseInt(document.getElementById("potenciaPanel").value)
    const anchoPanel = Number.parseFloat(document.getElementById("anchoPanel").value)
    const altoPanel = Number.parseFloat(document.getElementById("altoPanel").value)
    const pesoPanel = Number.parseFloat(document.getElementById("pesoPanel").value)
    const porcentajeAhorro = Number.parseFloat(document.getElementById("porcentajeAhorro").value)
    const limitacionEspacio = document.getElementById("limitacionEspacio").checked
    const areaDisponible = Number.parseFloat(document.getElementById("areaDisponible").value) || 0
    const pesoEstructura = Number.parseFloat(document.getElementById("pesoEstructura").value)

    // Obtener consumo actual
    const numCampos = tipoPeriodo === "mensual" ? 12 : 6
    const consumoActual = []
    for (let i = 0; i < numCampos; i++) {
      const valor = Number.parseFloat(document.getElementById(`consumo${i}`).value) || 0
    ...
    const valoresIrradiacion = Object.values(irradiacionMensual)
      ...
      .slice(0, -1)
    const hsp = valoresIrradiacion.reduce((sum, val) => sum + val, 0) / valoresIrradiacion.length
    ...
    if (limitacionEspacio && areaDisponible > 0) {
      ...
      numeroModulos = Math.ceil((consumoDiario * (porcentajeAhorro / 100) * 1000) / (hsp * potenciaPanel * 0.75))
    }
    ...
    const kwInstaladoConEficiencia = (potenciaPanel * numeroModulos * eficiencia) / 1000
    ...
    const produccionAnual = produccionMensual.reduce((sum, val) => sum + (isNaN(val) ? 0 : val), 0)
...
function mostrarResultados(resultados) {
  ...
  document.getElementById("consumoAnual").textContent = `${resultados.consumoAnual.toFixed(0)} kWh`
  document.getElementById("consumoDiario").textContent = `${resultados.consumoDiario.toFixed(2)} kWh`
  document.getElementById("consumoMensual").textContent = `${resultados.consumoMensual.toFixed(0)} kWh`
  document.getElementById("promedioConsumo").textContent = `${resultados.promedioConsumo.toFixed(0)} kWh`
  document.getElementById("hsp").textContent = `${resultados.hsp.toFixed(2)} horas`
  document.getElementById("potenciaFotovoltaica").textContent = `${resultados.PFV.toFixed(2)} kW`
  document.getElementById("numeroModulos").textContent = `${resultados.numeroModulos} módulos`
  document.getElementById("potenciaInstalada").textContent = `${resultados.potenciaInstalada.toFixed(2)} kW`
  document.getElementById("areaRequerida").textContent = `${resultados.areaRequerida} m²`
  document.getElementById("generacionAnual").textContent = `${resultados.produccionAnual.toFixed(0)} kWh`

  // Pesos del sistema
  document.getElementById("pesoTotalPaneles").textContent = `${resultados.pesoTotalPaneles.toFixed(0)} kg`
  document.getElementById("pesoPorM2").textContent = `${resultados.pesoPorM2.toFixed(1)} kg/m²`
  document.getElementById("pesoEstructura").textContent = `${resultados.pesoTotalEstructura.toFixed(0)} kg`
  document.getElementById("pesoTotalSistema").textContent = `${resultados.pesoTotal.toFixed(0)} kg`

  // Inversores
  document.getElementById("numeroInversores").textContent = `${Math.ceil(resultados.numeroInversores)} inversores`

  // Impacto ambiental
  document.getElementById("ahorroCO2").textContent = `${resultados.ahorroCO2.toFixed(3)} toneladas/año`
  document.getElementById("arbolesPlantados").textContent = `${Math.round(resultados.arbolesPlantados)} árboles`
  document.getElementById("promedioProduccion").textContent =
    `${resultados.promedioProduccionMensual.toFixed(0)} kWh/mes`
...
function llenarTablaDetalle(resultados) {
  ...
  const irradiacionArray = Object.values(resultados.irradiacionMensual).filter((v) => typeof v === "number")
  ...
  for (let i = 0; i < 12; i++) {
    ...
    row.insertCell(2).textContent = resultados.produccionMensual[i]?.toFixed(0) || "-"
    row.insertCell(3).textContent = resultados.consumoMensualExpandido[i]?.toFixed(0) || "0"
...
function exportResultsToPdf() {
  ...
  // Agregar encabezado profesional al clon
  ...
  header.innerHTML = `
        <div style="display: flex; align-items: center; justify-content: space-between; margin-bottom: 20px;">
            <div style="display: flex; align-items: center; gap: 15px;">
                <i class="fas fa-solar-panel" style="font-size: 2.5rem; color: #f59e0b;"></i>
                <div>
                    <h1 style="margin: 0; font-size: 2rem; font-weight: 800;">Hexa Solar Power Solutions</h1>
                    <p style="margin: 5px 0 0 0; opacity: 0.9;">Energía Solar Inteligente para un Futuro Sostenible</p>
                </div>
            </div>
        ...
        <div style="border-top: 1px solid rgba(255,255,255,0.3); padding-top: 20px;">
            <h2 style="margin: 0 0 15px 0; font-size: 1.5rem;">Análisis de Sistema Solar Fotovoltaico</h2>
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px; font-size: 0.95rem;">
                ...
                <div><strong>Ejecutivo de Ventas:</strong> ${nombreEjecutivo}</div>
                <div><strong>Fecha de Análisis:</strong> ${new Date().toLocaleDateString()}</div>
                <div><strong>Referencia:</strong> HSP-${Date.now().toString().slice(-6)}</div>
            </div>
        </div>
    `
  ...
  // Configuración mejorada para html2pdf con mejor manejo de páginas
  const opt = {
    margin: [15, 15, 15, 15],
    filename: `Hexa_Solar_Analisis_${nombreCliente.replace(/ /g, "_")}_${new Date().toISOString().split("T")[0]}.pdf`,
    ...
    pagebreak: {
      ...
      avoid: ".result-item, .card, .results-table",
    },
  }
  ...
  loadingText.textContent = "Generando reporte PDF profesional..."
